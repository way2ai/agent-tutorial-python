...
# 参数说明
# protocolVersion ：协议版本协商，确保客户端和服务器使用兼容的协议版本。
# capabilities：能力发现，声明其支持的功能，包括其能够处理的原始功能（工具、资源、提示）以及是否支持通知等特性。
# Identity Exchange：身份交换，clientInfo 和 serverInfo 提供标识和版本信息。
~~~
...
# name：该工具在服务器命名空间内的唯一标识符。
# title：该工具的人类可读显示名称。
# description：工具的详细说明以及使用时机。
# inputSchema：一个定义预期输入参数的 JSON Schema，实现类型验证并提供关于必需参数和可选参数的清晰文档说明。
~~~
...
# content.type 内容类型：每个内容对象都有一个 type 字段。在此示例中， "type": "text" 表示纯文本内容，但 MCP 支持多种内容类型以适应不同的使用场景。
~~~

4. Real-time Updates (Notifications) 实时更新 （通知）：MCP 支持实时通知，使服务器能够在未被明确请求的情况下向客户端通知变更。这展示了通知系统，这是保持 MCP 连接同步和响应的关键特性。

~~~json
# 当服务器可用的工具发生变化——例如新功能可用、现有工具被修改或工具暂时不可用时，服务器可以主动通知已连接的客户端。
# 服务器请求：
{
  "jsonrpc": "2.0",
...
# 请求参数解释：
# 无需响应：通知中没有 id 字段。这遵循 JSON-RPC 2.0 通知语义，即不期望或发送响应。
# 基于能力的：此通知仅由在初始化时在其工具能力中声明了 "listChanged": true 的服务器发送
# 事件驱动：服务器根据内部状态变化决定何时发送通知

# 接收到此通知后，客户端通常会通过请求更新的工具列表作出反应。
# 客户端请求： 
{
  "jsonrpc": "2.0",
  "id": 4,
...
此通知模式不仅适用于工具，还扩展到其他 MCP 基本操作，从而实现客户端与服务器之间的全面实时同步。